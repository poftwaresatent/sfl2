  ...
  quantizer(0)
{
  if(dimension == 0)
    ...
    cerr << "ERROR in Lookup::Lookup()\n"
	 << "   dimension previously given as " << dimension << endl
	 << "   and now given as " << _dimension << endl
	   ...
	   double to)
{
  ...
  cout << "ERROR in Lookup::PartialMin(" << from
       << ", " << to << ")\n   no minimum found\n";
  abort();
  return 0;
}
		    ...
		    double to)
{
  ...
  cout << "ERROR in Lookup::PartialMinInclusive(" << from
       << ", " << to << ")\n   no minimum found\n";
  abort();
  return 0;
}
